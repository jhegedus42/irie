...
import app.client.ui.routing.canBeRoutedTo.components.{NotWrapped_CacheTestRootComp_Backend, CacheTestComp, CacheTestRootCompProps, HomePage}
...
class Wrapper(toBeWrapped: Component[CacheTestRootCompProps, Unit, NotWrapped_CacheTestRootComp_Backend, CtorType.Props]) {
  ...
  lazy val wrapperConstructor =
    ScalaComponent
      ...
      .componentWillMount($ => {
        println ("we are creating the callback that will be " +
