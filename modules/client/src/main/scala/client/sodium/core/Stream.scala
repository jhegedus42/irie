...
class Stream[A] private (val node: Node, val finalizers: ListBuffer[Listener], val firings: ListBuffer[A]) {
  ...
  /**
    ...
    * returned [[Listener]] has a [[Listener.unlisten()* Listenerunlisten()]] method to cause the
    * listener to be removed. This is an OPERATIONAL mechanism is for interfacing between
    * the world of I/O and for FRP.
    *
    * @param handler The handler to execute when there's a new value.
    *                You should make no assumptions about what thread you are called on, and the
    *                handler should not block. You are not allowed to use [[BehaviorSink.send BehaviorSink.send(A)]]
    *                or [[StreamSink.send(a:A):Unit* StreamSink.send(A)]] in the handler.
  ...
  /**
    * A variant of [[core.Stream.listen(handler:A=>Unit):client.sodium\.Listener* listen(A=>Unit)]]  that handles the first
  ...
  /**
    * A variant of [[Stream.listen(handler:A=>Unit):client.sodium\.Listener* listen(A=>Unit)]] that will deregister the
    * listener automatically if the listener is garbage collected.
    * With [[Stream.listen(handler:A=>Unit):client.sodium\.Listener* listen(A=>Unit)]], the listener is
    * only deregistered if [[Listener.unlisten()* Listenerunlisten()]] is called explicitly.
    *
    * This method should be used for listeners that are to be passed to
    * [[core.Stream.addCleanup* Stream.addCleanup(Listener]] to ensure that things don't get kept alive
    * when they shouldn't.
    */
  ...
  /**
    ...
    *          [[core.Behavior.sample():A* Behavior.sample()]] in which case it is equivalent to
    *          [[core.Stream.snapshot[B]* Stream.snapshot(Behavior)]]ing the
  ...
  /**
    ...
    * visible as the cell's current value as viewed by [[core.Stream.snapshot[B,C]* Stream.snapshot(Behavior,(A,B)=>C)]]
    * until the following transaction. To put this another way,
    * [[core.Stream.snapshot[B,C]* Stream.snapshot(Behavior,(A,B)=>C)]] always sees the value of a cell as it was before
    * any client.state changes from the current transaction.
    */
  ...
  /**
    * A variant of [[core.Stream.hold(initValue:A)* hold(A)]] with an initial value captured by
  ...
  /**
    * Variant of [[core.Stream.snapshot[B,C]* snapshot(Behavior,(A,B)=>C)]] that captures the cell's value
  ...
  /**
    ...
    * [[core.Stream.hold(initValue:A)* hold(A)]] don't become visible as the cell's current value until the
    * following transaction. To put this another way, [[core.Stream.snapshot[B,C]* snapshot(Behavior,(A,B)=>C)]]
    * always sees the value of a cell as it was before any client.state changes from the current
    * transaction.
    */
  ...
  /**
    * Variant of [[core.Stream.snapshot[B,C]* snapshot(Behavior,(A,B)=>C)]] that captures the values of
    * two cells.
    */
  ...
  /**
    * Variant of [[Stream!.merge(s:client.sodium\.Stream[A],f:(A,A)=>A):client.sodium\.Stream[A]* merge(Stream,(A,A)=>A)]]
    ...
    * use [[Stream!.merge(s:client.sodium\.Stream[A],f:(A,A)=>A):client.sodium\.Stream[A]* Stream.merge(Stream,(A,A)=>A)]].
  ...
  /**
    ...
    *          [[core.Behavior.sample():A* Behavior.sample()]].
  ...
  /**
    * Transform an event with a generalized client.state loop (a Mealy machine). The function
    * is passed the input and the old client.state and returns the new client.state and output value.
    *
    * @param f Function to apply to update the client.state. It may construct FRP logic or use
    *          [[core.Behavior.sample():A* Behavior.sample()]] in which case it is equivalent to
    *          [[core.Stream.snapshot[B]* Stream.snapshot(Behavior)]]ing the
    *          cell. Apart from this the function must be <em>referentially transparent</em>.
    */
  ...
  /**
    * A variant of [[core.Stream.collect[B,S](initState:S,f:(A,S)=>(B,S)):client.sodium\.Stream[B]* collect(S,(A,S)=>(B,S)]]
    * that takes an initial client.state returned by [[Behavior.sampleLazy()* Behavior.sampleLazy()]].
    */
  ...
  /**
    * Accumulate on input event, outputting the new client.state each time.
    *
    * @param f Function to apply to update the client.state. It may construct FRP logic or use
    *          [[core.Behavior.sample():A* Behavior.sample()]] in which case it is equivalent to
    *          [[core.Stream.snapshot[B]* Stream.snapshot(Behavior)]]ing the
  ...
  /**
    * A variant of [[core.Stream.accum[S](initState:S,f:(A,S)=>S):client.sodium\.Behavior[S]* accum(S,(A,S)=>S)]] that takes an
    *  initial client.state returned by [[Behavior.sampleLazy()* Behavior.sampleLazy()]].
    */
  ...
  /**
    * Attach a listener to this stream so that its [[Listener.unlisten()* Listener.unlisten()]] is invoked
...
object Stream {
  ...
  /**
    * Variant of [[core.Stream.orElse(s:client.sodium\.Stream[A]):client.sodium\.Stream[A]* orElse(Stream)]]
    * that merges a collection of streams.
    */
  def orElse[A](ss: Iterable[Stream[A]]): Stream[A] = Stream.merge[A](ss, (_: A, right: A) => right)

  /**
    * Variant of [[Stream!.merge(s:client.sodium\.Stream[A],f:(A,A)=>A):client.sodium\.Stream[A]* Stream.merge(Stream,(A,A)=>A)]]
    * that merges a collection of streams.
    */
