...
class Behavior[A](val str: Stream[A], protected var currentValue: Option[A]) {
  ...
  /**
    ...
    * [[core.Stream.snapshot[B,C]* Stream.snapshot(Behavior,(A,B)=>C)]], [[Stream.filter Stream.filter(A=>Boolean)]] and
    * [[Stream!.merge(s:client.sodium\.Stream[A],f:(A,A)=>A):client.sodium\.Stream[A]* Stream.merge(Stream,(A,A)=>A)]]
  ...
  /**
    ...
    *
    * @see [[Stream!.holdLazy(initValue:client.sodium\.Lazy[A]):client.sodium\.Behavior[A]* Stream!.holdLazy()]]
    */
  ...
  /**
    ...
    * returned [[Listener]] has a [[Listener.unlisten()* Listener.unlisten()]] method to cause the
  ...
  /**
    * A variant of [[core.Behavior.listen(action:A=>Unit):client.sodium\.Listener* listen(A=>Unit)]] that will deregister
    * the listener automatically if the listener is garbage collected. With
    * [[core.Behavior.listen(action:A=>Unit):client.sodium\.Listener* listen(A=>Unit)]], the listener is
    * only deregistered if [[Listener.unlisten()* Listener.unlisten()]] is called explicitly.
    */
