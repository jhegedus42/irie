...
class Cell[A](val behavior: Behavior[A]) {
  ...
  /**
    ...
    * [[core.Stream.snapshot[B,C]* Stream.snapshot(Cell,(A,B)=>C)]], [[Stream.filter Stream.filter(A=>Boolean)]] and
  ...
  /**
    ...
    *
    * @see [[Stream!.holdLazy(initValue:client.sodium\.Lazy[A]):client.sodium\.Cell[A]* Stream!.holdLazy()]]
    */
  ...
  /**
    ...
    *               handler should not block. You are not allowed to use [[CellSink.send CellSink.send(A)]]
  ...
  /**
    * A variant of [[core.Cell.listen(action:A=>Unit):client.sodium\.Listener* listen(A=>Unit)]] that will deregister
    * the listener automatically if the listener is garbage collected. With
    * [[core.Cell.listen(action:A=>Unit):client.sodium\.Listener* listen(A=>Unit)]], the listener is
