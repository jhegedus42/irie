...
/**
  ...
  *          [[Behavior.sample():A* Behavior.sample()]].
...
class StreamSink[A](val f: (A, A) => A) extends StreamWithSend[A] {
  ...
  //TODO Scaladoc bug [[client.sodium.core.StreamSink(f:(A,A)=>A):client.sodium\.StreamSink[A]* StreamSink((A,A)=>A)]]
  /**
  ...
  /**
    ...
    * handlers registered with [[Stream.listen(handler:A=>Unit):client.sodium\.Listener* Stream.listen(A=>Unit)]] or
