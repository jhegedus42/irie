...
class Stream[A] private (val node: Node, val finalizers: ListBuffer[Listener], val firings: ListBuffer[A]) {
  ...
  /**
    * A variant of [[core.Stream.listen(handler:A=>Unit):sodium\.Listener* listen(A=>Unit)]]  that handles the first
  ...
  /**
    * A variant of [[core.Stream.collect[B,S](initState:S,f:(A,S)=>(B,S)):sodium\.Stream[B]* collect(S,(A,S)=>(B,S)]]
  ...
  /**
    * A variant of [[core.Stream.accum[S](initState:S,f:(A,S)=>S):sodium\.Behavior[S]* accum(S,(A,S)=>S)]] that takes an
...
object Stream {
  ...
  /**
    * Variant of [[core.Stream.orElse(s:sodium\.Stream[A]):sodium\.Stream[A]* orElse(Stream)]]
