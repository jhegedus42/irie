...
class Behavior[A](val str: Stream[A], protected var currentValue: Option[A]) {
  ...
  /**
    ...
    *
    * @see [[Stream!.holdLazy(initValue:sodium\.Lazy[A]):sodium\.Behavior[A]* Stream!.holdLazy()]]
    */
  ...
  /**
    * A variant of [[core.Behavior.listen(action:A=>Unit):sodium\.Listener* listen(A=>Unit)]] that will deregister
    * the listener automatically if the listener is garbage collected. With
    * [[core.Behavior.listen(action:A=>Unit):sodium\.Listener* listen(A=>Unit)]], the listener is
