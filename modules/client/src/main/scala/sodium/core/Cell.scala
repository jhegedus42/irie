...
class Cell[A](val behavior: Behavior[A]) {
  ...
  /**
    ...
    *
    * @see [[Stream!.holdLazy(initValue:sodium\.Lazy[A]):sodium\.Cell[A]* Stream!.holdLazy()]]
    */
  ...
  /**
    * A variant of [[core.Cell.listen(action:A=>Unit):sodium\.Listener* listen(A=>Unit)]] that will deregister
    * the listener automatically if the listener is garbage collected. With
    * [[core.Cell.listen(action:A=>Unit):sodium\.Listener* listen(A=>Unit)]], the listener is
