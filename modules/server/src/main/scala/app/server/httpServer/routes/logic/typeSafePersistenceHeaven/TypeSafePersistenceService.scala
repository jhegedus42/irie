package app.server.httpServer.routes.logic.typeSafePersistenceHeaven

import app.server.httpServer.routes.logic.typeSafePersistenceHeaven.typelessHell.StyxFacade
...
private[routes] case class TypeSafePersistenceService(
  ...
  ): Future[(AreWeHappyInTheUnderWorld,Entity[V])] = {
  ...
  def updateEntity[V <: EntityValue[V]: ClassTag]( entity: Entity[V] )(
    ...
    val res: Future[AreWeHappyInTheUnderWorld,Entity[V]] =
