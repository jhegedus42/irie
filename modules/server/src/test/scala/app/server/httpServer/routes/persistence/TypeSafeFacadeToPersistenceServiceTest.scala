...
import app.server.httpServer.routes.persistence.notTypeSafeWorld.state.StatePrintingUtils
...
class TypeSafeFacadeToPersistenceServiceTest extends FunSuite {
  ...
  test( "init" ) {
    ...
    def init(): Unit = {
      ...
      val pm = TypeSafeFacadeToPersistenceService(executionContext)
